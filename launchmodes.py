"""
##############################################################################
launch Python programs with command lines and reusable launcher scheme
classes; auto inserts "python" and/or path to Python executable at front of
command line; some of this module may assume "python" is on your system path
(see Launcher.py);
...
streams; multiprocessing module also is an option, but this is commandlines,
not functions: doesn't make sense to start a process which would just do one
of the options here;

new in this edition: runs script filename path through normpath() to change
any / to \ for Windows tools where required; fix is inherited by PyEdit and
others; on Windows, / is generally allowed for file opens, but not by all
launcher tools;
"""
...
class System(LaunchMode):
    """
    run Python script named in shell commandline
...
class Popen(LaunchMode):
    """
    run shell commandline in a new process
    caveat: may block caller, since pip closed too soon
    """
    def run(self, cmdline):
        cmdline = fixWindowsPath(cmdline)
...
class Start(LaunchMode):
    ...
    for Windows only: uses filename association
    """
    def run(self, cmdline):
